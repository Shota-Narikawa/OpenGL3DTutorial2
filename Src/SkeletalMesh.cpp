...
namespace Mesh {
	...
	/**
	* スケルタルメッシュに関するグローバルデータ及びその制御コードを格納する名前空間.
	*/
	namespace SkeletalAnimation {
		...
		namespace /* unnamed */ {
			...
			*/
			...
			bool isInitialized = false; ///< グローバルデータが初期化されていたらtrue.
			int currentUboIndex = 0; ///< UBOダブルバッファの書き込み側インデックス.
			UniformBufferPtr ubo[2]; ///< ボーン行列等の転送先となるUBO(ダブルバッファ).
			std::vector<uint8_t> uboData; ///< UBOに転送するデータを一時的に保存するためのバッファ.
			GLint uboOffsetAlignment = 0;

		} // unnamed namespace

		/**
		* スケルタルメッシュ制御用のグローバルデータを初期化する.
		*/
		bool Initialize()
		{
			if (!isInitialized) {
		...
		*/
		...
		/**
		* スケルタルメッシュの制御用データを追加する.
		*
		* @param data UBOに追加するデータのアドレス.
		* @param size UBOに追加するデータのバイト数.
		*
		* @retval 0以上 データが追加された位置を示すオフセット値.
		* @retval -1    追加に失敗.
		*/
		...
		*/
		...
		/**
		* スケルタルメッシュ制御用データをUBOバインディングポイントに割り当てる.
		*
		* @param offset これから描画するスケルタルメッシュのための、スケルタルメッシュ制御用データの位置.
		* @param size   スケルタルメッシュ制御用データのバイト数.
		*/
	...
	*/
	...
	/**
	* 指定されたフレームにおけるアニメーションデータを取得する.
	*
	* @param data  アニメーションデータ.
	* @param frame データを取得するフレーム.
	*
	* @return frameにおけるアニメーションデータ.
	*/
	template<typename T>
	...
	* グローバル変換行列を計算する.
	*
	* @param nodes    計算対象のノード群.
	* @param node     計算対象のノード.
	* @param animated 計算結果を格納する中間データ構造.
	*/
	...
	*
	* @param file      アニメーション対象のノード群を保持するファイル.
	* @param animation アニメーションデータ.
	* @param frame     データを取得するフレーム.
	*
	* @return 計算結果を格納する中間データ構造.
	*/
	...
	*/
	...
	/**
	* メッシュ要素を持つノードのリストを取得する.
	*
	* @param node 探索の起点となるノード.
	* @param list メッシュ要素を持つノードリストの作成先.
	*/
